...
Реализовано S3-совместимое хранилище аватарок на базе MinIO с загрузкой через буфер и стримы, валидацией типа и размера файла. Финансовый модуль поддерживает пополнение, вывод и переводы между пользователями — с пессимистичными блокировками, идемпотентностью и аудит-логом через сущность Transaction. Фоновые задачи выполняются через очередь Bull. Пользовательские запросы кэшируются в Redis с инвалидацией через EventEmitter.

Особое внимание уделено архитектуре и developer experience: чёткое разделение на слои (common, core, infra, feat), ENV-валидация через Zod, глобальные фильтры и логгер, pre-commit хуки через Husky, а также полная Swagger-документация.
...
└── feat/         # Доменные сущности (фичи): auth, account, avatar, balance, user, profile
```
